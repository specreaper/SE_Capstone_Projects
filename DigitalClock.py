...
def is_filesystem_writable():
    try:
        m = storage.getmount("/")   # CIRCUITPY filesystem
        return m.readonly
    except Exception as e:
        print("getmount failed:", e)
        return None
...
def is_first_5_mins():
    ...
    try:
        time_diff_minutes = (current_minutes - (last_bell_min))
    except:
        time_diff_minutes = 0
    try:
        time_diff_seconds = (current_seconds - (time_diff_minutes * 60))
    except:
        time_diff_seconds = 0
